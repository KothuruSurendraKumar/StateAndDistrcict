...
app.get("/states/:stateId/", async (request, response) => {
  ...
  const result = await database.get(stateQuery);
  response.send(convertStateDbObjectToResponseObject(result));
});
app.post("/districts/", async (request, response) => {
  const { stateId, districtName, cases, cured, active, deaths } = request.body;
  const addQuery = `INSERT INTO district (state_id,district_name,cases,cured,active,deaths)
...
app.get("/districts/:districtId/", async (request, response) => {
  ...
  const resultOne = await database.get(districtQuery);
  response.send(convertDistrictDbObjectToResponseObject(resultOne));
});
...
app.get("/states/:stateId/stats/", async (request, response) => {
  const { stateId } = request.params;
  const stateDistrictQuery = `SELECT 
      SUM(cases),
      SUM(cured),
      SUM(active),
      SUM(deaths)
       FROM district WHERE state_id = ${stateId};`;
  const stats = await database.get(stateDistrictQuery);
  response.send({
